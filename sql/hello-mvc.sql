...
-- rownum between 11 and 20 (offset이 있는 경우에는 rownum 쓰기가 어려움)
-- 때문에 rownum을 rnum으로 감싼 후 where절에 rnum으로 조건을 걸었음
...
-- 검색된 회원수 조회(totalContent)
select count(*) from member; -- 전체 대상
...
create table attachment(
        ...
        , constraint fk_attachmemt_board_no foreign key(board_no) references board(no) on delete cascade
);
...
create table board_comment (
        no number
        , comment_level number default 1 -- 댓글1, 대댓글2
        , writer varchar2(15)
...
values(seq_board_comment_no.nextval, default, 'abcde', '댓글작성 테스트', 221, null, default);

insert into board_comment
values(seq_board_comment_no.nextval, default, 'admin', '이달의 게시글 선정!!', 221, null, default);

insert into board_comment
values(seq_board_comment_no.nextval, default, 'sinsa', '구독과 좋아요 누르고 갑니다.', 221, null, default);

-- 221번 게시글 테스트 대댓글
insert into board_comment
values(seq_board_comment_no.nextval, 2, 'honggd', '저도 누르고 갑니다222', 221, 1, default); -- 1번 댓글에 대한 대댓글(comment_level과 comment_ref)

insert into board_comment
values(seq_board_comment_no.nextval, 2, 'qwerty', '테스트가 잘 되나요?', 221, 1, default);

insert into board_comment
values(seq_board_comment_no.nextval, 2, 'sinsa', '게시글 잘 보고갑니다', 221, 2, default);
commit;
...
-- start with 최상위레코드 조건절(여러개일 수 있다)
...
where
        board_no = 221
start with
        comment_level = 1 -- 부모레코드
connect by
        prior no = comment_ref -- 부모레코드의 no와 comment_ref와 번호가 일치하다면 계층형으로 보여줘라
order siblings by
        no asc;
...
-- delete from board_comment where no in (??);

-- 게시글 목록 조회 - 댓글개수 포함
-- select * from (select row_number() over(order by no desc) rnum, b.*, (select count(*) from attachment where board_no = b.no) attach_count, (select count(*) from board_comment where board_no = b.no) view_count from board b) b where rnum between ? and ?
select
        *
from
        (
        ...
        b.*
        , (select count(*) from attachment where board_no = b.no) attach_count
        , (select count(*) from board_comment where board_no = b.no) view_count from board b) b
where rnum between 1 and 5;

select count(*) from board_comment where board_no = 221; 
